...
let EdificiosService = class EdificiosService {
    ...
    async getAll() {
        ...
        const result = await pool.request().query(`SELECT * FROM dbo.tblEdificios WHERE Activo = 1 ORDER BY Nombre`);
    ...
    async create(dto) {
        ...
        const result = await request.query(`INSERT INTO dbo.tblEdificios (${cols.join(', ')}) OUTPUT INSERTED.* VALUES (${params.join(', ')})`);
    ...
    async update(id, dto) {
        ...
        const result = await request.query(`UPDATE dbo.tblEdificios SET ${sets.join(', ')} OUTPUT INSERTED.* WHERE Id = @id`);
...
//# sourceMappingURL=edificios.service.js.map