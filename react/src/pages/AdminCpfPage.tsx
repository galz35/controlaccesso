...
export default function AdminCpfPage() {
  ...
  const [form, setForm] = useState({ username: '', password: '', nombre: '', tipo: 'PROVEEDOR', referenciaId: '', edificioIdDefecto: '' });
  const [showPwd, setShowPwd] = useState(false);

  const load = useCallback(async () => {
    setLoading(true); setApiError(false);
    try {
      const [uRes, eRes] = await Promise.all([api.get('/admin/cpf-users'), api.get('/edificios')]);
      setUsers(uRes.data || []); setEdificios(eRes.data || []);
  ...
  const crear = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/auth/cpf-register', { username: form.username, password: form.password, nombre: form.nombre, tipo: form.tipo, referenciaId: form.referenciaId ? parseInt(form.referenciaId) : undefined, edificioIdDefecto: form.edificioIdDefecto ? parseInt(form.edificioIdDefecto) : undefined });
      showSuccess('Usuario CPF creado');
      setShowForm(false); setForm({ username: '', password: '', nombre: '', tipo: 'PROVEEDOR', referenciaId: '', edificioIdDefecto: '' }); load();
        ...
        <form onSubmit={crear} className="card mb-3">
          <div className="card__body">
            <div className="form-grid form-grid--3">
              ...
              <div className="form-group"><label htmlFor="cpf-edif" className="form-label">Edificio <span className="form-hint">(opcional, preseleccionado al registrar acceso)</span></label>
                <select id="cpf-edif" className="form-control" value={form.edificioIdDefecto} onChange={e => setForm({...form, edificioIdDefecto: e.target.value})}>
                  <option value="">Sin edificio fijo</option>
