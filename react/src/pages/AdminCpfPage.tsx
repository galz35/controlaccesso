...
export default function AdminCpfPage() {
  ...
  const crear = async () => {
    if (!form.username || !form.password || !form.nombre) { Swal.fire({ icon: 'error', title: 'Campos requeridos', text: 'Complete todos los campos.' }); return; }
    if (form.password.length < 6) { Swal.fire({ icon: 'error', title: 'Contraseña muy corta', text: 'Mínimo 6 caracteres.' }); return; }
    try {
    ...
    } catch (err: any) { Swal.fire({ icon: 'error', title: 'Error', text: err?.response?.data?.message || 'Error al crear' }); }
  };
  ...
  const resetPassword = async (username: string) => {
    ...
    try { await api.put('/auth/cpf-password', { username, oldPassword: '', newPassword: newPwd }); Swal.fire({ icon: 'success', title: 'Contraseña cambiada', timer: 2000, showConfirmButton: false }); } catch (err: any) { Swal.fire({ icon: 'error', title: 'Error', text: err?.response?.data?.message || 'Error' }); }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-header__title"><KeyRound className="icon v-middle" /> Usuarios Externos (CPF)</h1>
        ...
        <div className="card mb-3">
          <div className="card__body">
            <div className="form-grid form-grid--3">
              <div className="form-group"><label className="form-label form-label--required">Usuario</label><input type="text" className="form-control" value={form.username} onChange={e => setForm({...form, username: e.target.value})} /></div>
              <div className="form-group"><label className="form-label form-label--required">Contraseña <span className="form-hint">(mín. 6 carac.)</span></label>
                <div className="password-field"><input type={showPwd ? 'text' : 'password'} className="form-control" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
              ...
              <div className="form-group"><label className="form-label form-label--required">Nombre</label><input type="text" className="form-control" value={form.nombre} onChange={e => setForm({...form, nombre: e.target.value})} /></div>
              <div className="form-group"><label className="form-label">Tipo</label><select className="form-control" value={form.tipo} onChange={e => setForm({...form, tipo: e.target.value})}><option value="PROVEEDOR">Proveedor</option><option value="INSTRUCTOR_EXTERNO">Facilitador Externo</option></select></div>
              <div className="form-group"><label className="form-label">ID Referencia <span className="form-hint">(opcional)</span></label><input type="number" className="form-control" value={form.referenciaId} onChange={e => setForm({...form, referenciaId: e.target.value})} /></div>
            </div>
            <div className="form-actions"><button onClick={crear} className="btn btn--primary">Guardar</button><button onClick={() => setShowForm(false)} className="btn btn--secondary">Cancelar</button></div>
          </div>
        </div>
      )}
