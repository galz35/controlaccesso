    ...
    <div>
      ...
      <div className="kpi-grid">
        ...
        <div className="kpi-card kpi-card--green">
          <Building2 className="icon--md kpi-card__icon" />
        ...
        <div className="card__body card__body--flush">
          {loading ? (
            ...
            <div className="empty-state">
              <DoorOpen className="icon--lg empty-state__icon" aria-hidden="true" />
              <p className="empty-state__desc">No hay accesos registrados hoy</p>
            </div>
          ) : (
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th scope="col" className="table td--center">Entrada</th>
                    <th scope="col" className="table td--center">Salida</th>
                ...
                <tbody>
                  {hoy.map(r => (
                    <tr key={r.id}>
                      ...
                      <td className="table td--bold">{r.nombre}</td>
                      <td className="table td--muted">{r.cedula || '-'}</td>
                      <td className="table td--muted">{r.edificio}</td>
                      <td className="table td--center table td--small">{new Date(r.fechaEntrada).toLocaleTimeString()}</td>
                      <td className="table td--center">
                        {r.fechaSalida ? (
                          <span className="badge badge--neutral">{new Date(r.fechaSalida).toLocaleTimeString()}</span>
                        ) : (
                      ...
                      <td className="table td--center">
                        {!r.fechaSalida && (
                          <button onClick={() => registrarSalida(r.id)} className="btn btn--dark btn--sm">
